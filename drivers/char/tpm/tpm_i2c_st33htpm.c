...
static int tpm_tis_i2c_read_bytes(struct tpm_tis_data *data, u32 addr,
				  u16 size, u8 *result, enum tpm_tis_io_mode io_mode)
{
...
static int tpm_tis_i2c_read16(struct tpm_tis_data *data, u32 addr, u16 *result)
{
	...
	rc = data->phy_ops->read_bytes(data, addr, sizeof(u16), (u8 *)result, TPM_TIS_PHYS_16);
	if (!rc)
...
static int tpm_tis_i2c_write32(struct tpm_tis_data *data, u32 addr, u32 value)
{
	...
	return data->phy_ops->write_bytes(data, addr, sizeof(u32),
					   (u8 *)&value, TPM_TIS_PHYS_32);
}

static int tpm_tis_i2c_check_data(struct tpm_tis_data *data, size_t len, const u8 *buf)
{
	struct tpm_tis_i2c_phy *phy = to_tpm_tis_i2c_phy(data);
	u16 crc, crc_tpm;
	if (phy->data_csum==1) {
...
static const struct tpm_tis_phy_ops tpm_tis = {
	...
	.verify_crc = tpm_tis_i2c_check_data,
};
...
static struct i2c_driver tpm_i2c_st33htpm_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "tpm_i2c_st33htpm",
		.pm = &tpm_tis_i2c_pm,
...
module_i2c_driver(tpm_i2c_st33htpm_driver);
